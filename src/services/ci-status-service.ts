...
import { CircleciService, CircleWorkflow } from './circleci-service'
...
interface PipelineStatus {
    pipelineStatus: 'not_found' | 'running' | 'failed' | 'succeeded'
...
export class CiStatusService {
    ...
    private async getPipelineStatus(branchName: string, gitRemoteUrl: string): Promise<PipelineStatus> {
        ...
        return {
            pipelineStatus: failedWorkflows.length ? 'failed' : incompleteWorkflows.length ? 'running' : 'succeeded',
    ...
    private async getFailedJobs(failedWorkflows: CircleWorkflow[], projectSlug: string): Promise<CiJobStatus[]> {
        const jobs = await Promise.all(failedWorkflows.map(
            workflow => this.circleciService.getWorkflowJobs(workflow.id).then(jobs => jobs.map(
                job => ({ ...job, workflow }),
            )),
        ))
        const baseUrl = 'https://app.circleci.com/pipelines/' + projectSlug.replace(/^gh/, 'github')
        return flatten(jobs)
